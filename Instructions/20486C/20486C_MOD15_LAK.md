...
a. go to **Allfiles(D):\Mod11\Labfiles\PhotoSharingApplication_11_begin\PhotoSharingApplication**, and then copy the **web.config** file.  
b. Go to **Allfiles(D):\Mod13\Labfiles\PhotoSharingApplication_15_begin\PhotoSharingApplication**, and then paste the **web.config** file.  
...
2. In the **Libraries** window, navigate to **Allfiles(D):\Mod15\Labfiles\Starter\PhotoSharingApplication**, and then double-click **PhotoSharingApplication.sln**.
...
```
5. In the **Solution Explorer** pane of the **PhotoSharingApplication - Microsoft Visual Studio** window, under **PhotoSharingApplication**, expand **References**.

  >**Note:** The NuGet package added two **Microsoft.aspnet.SignalR** namespaces to the project references.

6. In the **Solution Explorer** pane, under **PhotoSharingApplication**, expand **Scripts**.

  >**Note:** The NuGet package added severn **jquery** scripts and two **signalR** scripts, to the **Scripts** folder.
...
1. In the **Solution Explorer** pane, right click **PhotoSharingApplication** project and click **Add** and then **New class**.
2. Name the class **Startup.cs** and click **Add**.
3. Locate ```using System.Web;``` and add the following using statemets just after:
  ```cs
    using Owin;
